...
<html>
  <head>
    <meta charset="utf-8">
    <title>Japan Videoes</title>
  </head>	
  <body bgcolor="lightyellow">
    <p align='center'><a href="jpvideo.php">Back</a></p>
    <table border="0" align="center" width="1400" cellspacing="2"> 
      <tr bgcolor="#BABA76" height="30" align="center">
        ...
        <?php
          ...
          //篩選出所有產品資料
          //$sql = "SELECT video_id, title, channel_title, category_id, publish_time, trending_date, views, likes, comment_count FROM JPvideos";
          //$sql = "SELECT video_id, title, channel_title, category_id, publish_time, trending_date, views, likes, comment_count FROM JPvideos WHERE category_id=" . $category . " AND views>=" . $view1 . " AND views<=" . $view2 ." AND likes>=" . $like1 . " AND likes<=" . $like2 . " AND comment_count>=" . $comment1 . " AND comment_count<=" . $comment2;
          if (!publish){
          $sql = "SELECT v.video_id, v.title, v.channel_title, c.category, v.publish_time, v.trending_date, v.views, v.likes, v.comment_count FROM JPvideos v,  (select category_id,category from JPcategory where category_id=" . $category . ") c WHERE v.category_id=" . $category . " AND v.views>=" . $view1 . " AND v.views<=" . $view2 ." AND v.likes>=" . $like1 . " AND v.likes<=" . $like2 . " AND v.comment_count>=" . $comment1 . " AND v.comment_count<=" . $comment2 . " ORDER BY v.publish_time ASC";
          }else{
